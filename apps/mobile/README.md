...
9. **文案里不得出现 Markdown 标记**（`**强调**`、行内反引号）。
   RN 的 `<Text>` 把 children 当**纯文本**渲染，不做 Markdown 解析 ——
   写了 `**这样**`，用户在界面上读到的是 `**这样**`，两个星号原样显示。

   为什么单列一条：它**不会让任何现有防线失败**。`tsc` 只查类型、不管字符串内容；
   单元测试不渲染界面；离线快照（`docs/ui-render/`）只回传**布局诊断**
   （横向溢出量、未捕获 JS 错误），不核对正文里有没有多余字符。
   仓库里曾同时存在 **63 对**标记（`src/content/help.ts` 独占 53 对 ——
   那是每页右上角「讲解」抽屉的全部正文），而三个防线全程绿灯。

   同类缺陷在本仓库**已经发生过两次**：先管理台（`static/admin.html`），
   后 App 侧。守卫见 `tests/mobile/test_ui_copy_plain_text.py`。
   （对应 RULE-005「规则不散落」）。

...
1. 🔴 **真机 UI 走查 —— 现在排第 1 位。** 唯一能证伪「前端 APP 能实战」的手段：
   `tsc` 与 `expo export` 都**证不了**布局溢出 / 窄屏换行 / 键盘遮挡 / 「3 秒内确认」手感。
   V2 又新增 3 个页面（首页改版 / adjust / sensors），并有两轨配色要一起看。
   逐项清单见 §7.1。顺带可补 `/adjust` 的讲解入口（§2.5 遗留）
...
### 7.1 🔴 未验证项清单（真机走查时逐项过，别当成"已经验过了"）

离线能验的都已验（`tsc` 零错、`expo export` 出包并核到字节码字符串、接口契约、
真实 HTTP 的 62 项断言、`tests/mobile` 248 项、两处变异验证）。
但下面这些**只能在真机/模拟器上确认**，历轮**都没有做**：
...
| 「3 秒内确认」的真实体感 | 只有真机能测 |
| 键盘弹起是否遮挡输入框 | 同上 |
| **V2** `/sensors` 路由是否跳得过去、深色 header 是否生效 | 类型检查不校验路由；export 只证明文件进了包 |
| **V2** `/sensors` 曲线在真机是否真有数据、静止时是否接近水平 | 模拟器无传感器数据；离线只能验几何（17 条锚点） |
| **V2** 三张指标卡 / 三轴表在窄屏是否挤成两行 | 没有渲染器 |
| **V2** 深色仪器风与浅色页并存的观感（罗盘域 ⇄ 命盘域来回切） | 双轨配色只能眼睛看 |
| **V2** `/adjust` 深色盘的对比度是否够（金线/朱针压在深藏青上） | 同上 |

...
- **Phase 0~3 全部落地**，**1372 测全绿（2 skipped）**
...
- **真机包已交付**：`dist/玄盘AI-v0.1.0-sanshi-taiyi.apk`（94MB，2026-09-17 21:09 重打）
  —— 含三式全部（奇门+六壬+太乙）；已验 bundle 内含 `taiyi/cast`、`taiyi/meta` 路由。
  旧包 `玄盘AI-v0.1.0-sanshi.apk`（太乙落地前）已过时。
  签名用的是 **debug keystore**，对外分发前必须换正式签名
- 🔴 `[未验证]` **真机 UI 走查仍未做** —— 这是「能实战」唯一未被证伪的一环
- 无 remote 仓库

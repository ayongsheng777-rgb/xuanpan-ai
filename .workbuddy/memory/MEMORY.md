...
## 现状（2026-09-18 08:10 · V2 改造中断点）

- **🔴 当前主线 = V2 产品改造（演示图深色仪器风），进行中断点交接**
  —— 已完成 6 commit（评估文档 / instrument 色域 / CompassDial 双轨 /
  sensorQuality 库+22测 / 罗盘首页深色 / /adjust 调节页），
  **剩 `/sensors` 页 + 验证 + 文档提交**。**接手先读 HANDOFF §2.5 与
  `docs/玄盘 AI — V2 评估与实施路线.md`**。基线 tag `v0.1.0-pre-v2`。
- **Phase 0~3 全部落地**，**1372 测全绿（2 skipped）**（V2 传感器 +22 后应为 1394，断点未重跑）
  （基线演进：626 → 765 → 786 → 834 → 940 → 1175 → 1186 → 1372）
- 术数能力已补齐：六爻装卦层、八字大运/流年/神煞/长生十二宫、黄历/择日、
  **断卦层**（`duangua.py`：六爻/八字吉凶倾向 + 流派标注）、
  **择日决策**（`zeri.py`：17 事件反推吉日，分级非断语 + 流派模块化）、
  **三式全部：奇门遁甲 + 大六壬 + 太乙神数**（`qimen/`、`liuren/`、`taiyi/`，
  各自贯通内核 → HTTP → MCP → 管理台 → APP）
  （详见 `docs/玄盘 AI — 术数能力追赶路线图.md`；**三式空白清零**）
- **康熙笔画字库已扩至 20794 字**（`verified:true`，权威源 shunshi-kangxi-core）
- **MCP 暴露层**：`services/mcp/server.py`，**12 个工具**，structuredContent +
  纯 stdio 不弹浏览器；测试走真实 stdio 子进程
- **HTTP 层**：`/api/v1/{almanac,zeri,duan,qimen,liuren,taiyi}`；
  **管理台 `/admin`** 13 张查询面板（概览 / 会话 / 黄历 / 择日 / 六爻 / 八字 /
  奇门 / 大六壬 / 太乙 + 黄历区间 / 择日单日 / 姓名 / 求签 / 六爻排盘 / 坐向），
  令牌鉴权、安全默认关
- **APP**：黄历择日页、共用断卦展示层 `DuanCard`、**三式排盘页 `/sanshi`**
  （奇门 + 六壬共用「起局时刻」、太乙独立「起局年份」）、罗盘盘面仿真、每页讲解入口；
  **V2：罗盘首页深色仪器风 + `/adjust` 调节页**（`/sensors` 待收尾）
- **真机包已交付**：`dist/玄盘AI-v0.1.0-sanshi-taiyi.apk`（94MB，2026-09-17 21:09 重打）
  —— 含三式全部；⚠️ **不含 V2 罗盘域改版**，V2 验证后需重打。
  签名用的是 **debug keystore**，对外分发前必须换正式签名
- 🔴 `[未验证]` **真机 UI 走查仍未做** —— V2 又增 3 个新页面，更必要了
- **remote 已建**：`https://github.com/ayongsheng777-rgb/xuanpan-ai`（私有），main 推至 `e90f135`
...
### 术数追赶下一步（2026-09-17 三式收官，算法空白清零）

已落地：MCP 暴露层 / 康熙笔画字库 / 断卦层 / **择日决策** / **查询域 HTTP + 管理台 + 前端接线** /
**三式全部（奇门/六壬/太乙，全链路）**。剩余按性价比：

1. **真机 UI 走查** —— 第 1 位：唯一能证伪「前端 APP 能实战」的手段，
...
5. 太乙进阶（月局/日局/时局）—— 已显式列入 uncertainties，价值高但算法难，长线挂起
